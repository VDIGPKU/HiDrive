...
LEADERBOARD_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
def get_weather_id(weather_conditions):
    ...
    weather_xml = os.path.join(LEADERBOARD_ROOT, 'data', 'weather.xml')
...
class LeaderboardEvaluator(object):
    ...
    def __init__(self, args, statistics_manager):
        ...
        self._base_agent_config = args.agent_config
    ...
    def _setup_simulation(self, args):
        ...
        if args.use_existing_server:
            print(
                f"Reuse mode enabled, using existing CARLA server at {args.host}:{args.port} "
                f"(TM port: {args.traffic_manager_port})",
                flush=True
            )
        else:
            ...
            env['__VK_ICD_FILENAMES'] = '/usr/share/vulkan/icd.d/nvidia_icd.json'

            self.server = subprocess.Popen(cmd1, shell=True, preexec_fn=os.setsid, env=env)
        ...
        while attempts < num_max_restarts:
            ...
            raise RuntimeError(
                f"Failed to connect/load world at {args.host}:{args.port} after {num_max_restarts} attempts"
            )
        attempts = 0
        num_max_restarts = 40
        while attempts < num_max_restarts:
            ...
            raise RuntimeError(
                f"Failed to initialize traffic manager at port {args.traffic_manager_port} "
                f"after {num_max_restarts} attempts"
            )
    ...
    def _load_and_wait_for_world(self, args, town):
        ...
        for m in available_maps:
            if town in m or m.endswith(town):
                target_map = m
                break

        if target_map is None:
            target_map = town
            print(f"Warning: Map '{town}' not found in available maps, trying anyway")

        print(f"Loading map: {target_map}")

        try:
            self.world = self.client.load_world(target_map, reset_settings=False)
        except TypeError:
            print("Trying load_world without reset_settings parameter...")
            self.world = self.client.load_world(target_map)
        except Exception as e:
            print(f"load_world failed: {e}, trying to use current world")
        ...
        try:
            ...
            print("Large map settings not supported in this CARLA version")
        self.world.apply_settings(settings)
        ...
        if town not in map_name and map_name not in town:
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        try:
            ...
            agent_config_for_route = self._base_agent_config + '+' + save_name
            self.agent_instance.setup(agent_config_for_route)
...
def main():
    ...
    parser.add_argument('--use-existing-server', action='store_true',
                        help='Reuse an already running CARLA server instead of launching a new one')
