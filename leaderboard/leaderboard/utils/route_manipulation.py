...
def interpolate_trajectory(waypoints_trajectory, hop_resolution=1.0):
    ...
    # Guard for dense manual routes: some adjacent points may snap to opposite/invalid lanes.
    # In these rare short-hop cases, the planner can produce huge detours (hundreds of meters),
    # which explodes route visualization and route length.
    short_hop_max_dist = 8.0
    short_hop_max_detour = 150.0
    short_hop_max_ratio = 20.0
    ...
    for i in range(len(waypoints_trajectory) - 1):
        ...
        if interpolated_trace:
            euclidean_dist = waypoint.distance(waypoint_next)
            if euclidean_dist < short_hop_max_dist:
                road_dist = 0.0
                prev_loc = interpolated_trace[0][0].transform.location
                for wp, _ in interpolated_trace[1:]:
                    curr_loc = wp.transform.location
                    road_dist += curr_loc.distance(prev_loc)
                    prev_loc = curr_loc

                detour_ratio = road_dist / max(euclidean_dist, 1e-3)
                if road_dist > short_hop_max_detour or detour_ratio > short_hop_max_ratio:
                    start_wp = world_map.get_waypoint(waypoint, project_to_road=True)
                    end_wp = world_map.get_waypoint(waypoint_next, project_to_road=True)
                    if start_wp is not None and end_wp is not None:
                        interpolated_trace = [
                            (start_wp, RoadOption.LANEFOLLOW),
                            (end_wp, RoadOption.LANEFOLLOW),
                        ]
