...
class RouteParser(object):
    ...
    def is_scenario_at_route(trigger_transform, route):
        ...
        for route_transform, _ in route:
            ...
            if dpos < min_dist:
                min_dist = dpos
                closest_wp = route_transform

        if closest_wp:
            dyaw = (float(trigger_transform.rotation.yaw) - closest_wp.rotation.yaw) % 360
            print(f"[DEBUG] Closest waypoint distance: {min_dist:.2f}m, waypoint yaw: {closest_wp.rotation.yaw:.1f}, trigger yaw: {trigger_transform.rotation.yaw}, dyaw: {dyaw:.1f}")
