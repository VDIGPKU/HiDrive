...
class MalfunctionSignalJunctionRightTurn(JunctionRightTurn):
    """
    Traffic light malfunction version of JunctionRightTurn.
    All traffic lights at the junction are turned off (dark / not lit).
    The ego must treat it as an uncontrolled intersection.
    """
    ...
    def _initialize_actors(self, config):
        ...
        black_tex = carla.TextureColor(16, 16)
        black_pixel = carla.Color(0, 0, 0, 255)
        for y in range(16):
            for x in range(16):
                black_tex.set(x, y, black_pixel)
        for tl in self._frozen_tls:
            try:
                tl.apply_texture(carla.MaterialParameter.Emissive, black_tex)
            except Exception:
                pass

        self._world.tick()
        print(f"  [INFO] Turned off {len(self._frozen_tls)} traffic lights")

    def _create_behavior(self):
        sequence = py_trees.composites.Sequence(name="MalfunctionSignalJunctionRightTurn")
        if self.route_mode:
            sequence.add_child(HandleJunctionScenario(
                clear_junction=True,
                clear_ego_entry=True,
