...
class ScenarioNotification(py_trees.behaviour.Behaviour):
    """Print a one-time scenario notification."""
...
class PedestrianCrossing(BasicScenario):
    ...
    def _initialize_actors(self, config):
        ...
        print(f"\n{'='*60}")
        print(f"  🚶 SCENARIO LOADED: PedestrianCrossing")
        print(f"  Trigger location: {self._trigger_location}")
        print(f"  Pedestrian count: {len(self._walker_data)}")
        print(f"{'='*60}\n")
    ...
    def _create_behavior(self):
        ...
        sequence.add_child(ScenarioNotification(
            "SCENARIO TRIGGERED: PedestrianCrossing - pedestrians are crossing the road; slow down and yield."))
