...
class TimedTrafficLightMalfunction(py_trees.behaviour.Behaviour):
    """
    Keep a list of traffic lights in OFF state for a fixed game-time duration,
    then restore each one to the state/timing captured at start.
    The behavior keeps RUNNING so it does not end its parent parallel node early.
    """

    def __init__(self, traffic_lights, duration=20.0, name="TimedTrafficLightMalfunction"):
    ...
    def initialise(self):
        ...
        for tl in self._traffic_lights:
            try:
                ...
                hold_time = self._duration + elapsed_time
                tl.set_green_time(hold_time)
                tl.set_red_time(hold_time)
                tl.set_yellow_time(hold_time)
            except RuntimeError:
                continue
    ...
    def _restore(self):
        if self._restored:
            return
        for tl, info in list(self._previous_traffic_light_info.items()):
            try:
    ...
    def update(self):
        if not self._restored and GameTime.get_time() - self._start_time > self._duration:
            self._restore()
...
class MalfunctionSignalJunctionLeftTurnEnterFlow(JunctionLeftTurnEnterFlow):
    """
    Traffic light malfunction version of 'JunctionLeftTurn'.
    All traffic lights at the junction are turned off (dark / not lit),
    simulating a signal failure. The ego must treat it as an uncontrolled
    intersection and yield to the crossing traffic flow.
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self.activate_scenario = True
        self._off_tl_dict = {}
        self._malfunction_duration = get_value_parameter(config, 'malfunction_duration', float, 20.0)
        super().__init__(world, ego_vehicles, config, randomize, debug_mode, criteria_enable, timeout)

    def _initialize_actors(self, config):
        super()._initialize_actors(config)

        # Dump ALL traffic lights in the world for debugging
        all_tls = list(self._world.get_actors().filter('traffic.traffic_light*'))
        ego_loc = self._ego_wp.transform.location
        print(f"  [DEBUG] Ego wp location: {ego_loc}")
        print(f"  [DEBUG] ALL {len(all_tls)} traffic lights in world:")
        for tl in sorted(all_tls, key=lambda t: t.get_location().distance(ego_loc)):
            dist = tl.get_location().distance(ego_loc)
            print(f"  [DEBUG]   id={tl.id} type={tl.type_id} loc={tl.get_location()} dist={dist:.1f}m state={tl.get_state()}")

        # Just turn off ALL traffic lights in the entire map
        self._frozen_tls = all_tls
        if not self._frozen_tls:
            print("  [WARN] No traffic lights found")
        else:
            print(f"  [INFO] Will turn off ALL {len(self._frozen_tls)} traffic lights for {self._malfunction_duration:.1f}s")

    def _create_behavior(self):
        sequence = py_trees.composites.Sequence(name="MalfunctionSignalJunctionLeftTurnEnterFlow")

        root = py_trees.composites.Parallel(policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        root.add_child(TimedTrafficLightMalfunction(
            self._frozen_tls,
            duration=self._malfunction_duration
        ))
        if self.activate_scenario:
            end_condition = py_trees.composites.Sequence()
            end_condition.add_child(WaitEndIntersection(self.ego_vehicles[0]))
        ...
        # No TrafficLightFreezer needed — lights are already frozen via freeze() API
    ...
    def __del__(self):
        # Ensure no traffic light remains frozen in abnormal exits.
        if hasattr(self, '_frozen_tls'):
            for tl in self._frozen_tls:
                try:
                    tl.freeze(False)
                except RuntimeError:
                    pass
