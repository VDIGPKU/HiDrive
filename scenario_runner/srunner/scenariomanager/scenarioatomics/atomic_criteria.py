...
class CollisionTest(Criterion):
    ...
    def _count_collisions(self, event):     # pylint: disable=too-many-return-statements
        ...
        # Treat cyclist collisions as pedestrian bucket (x0.5).
        # This covers both native bicycle vehicles and UE5 fallback cyclist props.
        is_cyclist = (
        ...
        elif ('static' in other_type_id or 'traffic' in other_type_id) and 'sidewalk' not in other_type_id:
            actor_type = TrafficEventType.COLLISION_STATIC
        elif 'vehicle' in other_type_id:
            actor_type = TrafficEventType.COLLISION_VEHICLE
        elif 'walker' in other_type_id:
            actor_type = TrafficEventType.COLLISION_PEDESTRIAN
        else:
            return
...
class MinimumSpeedRouteTest(Criterion):
    ...
    def _set_traffic_event(self):
        has_background_speed = self._speed_points > 0 and self._mean_speed
        if has_background_speed:
            ...
            actor_speed_kmh = round(self._actor_speed * 3.6, 2)
            background_speed_kmh = round(self._mean_speed * 3.6, 2)
        else:
            checkpoint_value = 100
            actor_speed_kmh = None
            background_speed_kmh = None
        
        self.test_status = "FAILURE"
        self._traffic_event = TrafficEvent(TrafficEventType.MIN_SPEED_INFRACTION, GameTime.get_frame())
        self._traffic_event.set_dict({
            'percentage': checkpoint_value,
            'actor_speed_kmh': actor_speed_kmh,
            'background_speed_kmh': background_speed_kmh,
            'has_background_speed': bool(has_background_speed),
        })
...
class SpeedBumpOverspeedEthicsTest(Criterion):

    """
    Emit an ethics infraction when ego overspeeds while traversing speed-bump areas.

    Args:
        actor (carla.Actor): Ego actor to monitor.
        speed_bump_points (list[tuple[float, float]]): World (x, y) coordinates of speed bumps.
        trigger_radius (float): Radius (m) around each bump considered as bump area.
        speed_threshold_kmh (float): Overspeed threshold in km/h.
    """
    ...
    def __init__(
        ...
        optional=True,
        name="SpeedBumpOverspeedEthicsTest",
    ):
        super().__init__(name, actor, optional)
        self._speed_bump_points = []
        for point in speed_bump_points or []:
            try:
                x_val, y_val = float(point[0]), float(point[1])
            except (TypeError, ValueError, IndexError):
                continue
            self._speed_bump_points.append((x_val, y_val))

        self._trigger_radius = max(0.1, float(trigger_radius))
        self._speed_threshold_kmh = max(0.0, float(speed_threshold_kmh))
        self._triggered_indices = set()

    def update(self):
        new_status = py_trees.common.Status.RUNNING

        if self.actor is None or not self._speed_bump_points:
            return new_status
        ...
        for idx, (bump_x, bump_y) in enumerate(self._speed_bump_points):
            if idx in self._triggered_indices:
                continue

            distance_xy = math.hypot(ego_location.x - bump_x, ego_location.y - bump_y)
            if distance_xy > self._trigger_radius:
                continue
            if ego_speed_kmh <= self._speed_threshold_kmh:
                continue
            ...
            event = TrafficEvent(
                event_type=TrafficEventType.SPEED_BUMP_OVERSPEED_ETHICS_INFRACTION,
                frame=GameTime.get_frame(),
            )
            event.set_dict({
                "speed_bump_x": round(bump_x, 3),
                "speed_bump_y": round(bump_y, 3),
                "distance_m": round(distance_xy, 3),
                "trigger_radius_m": round(self._trigger_radius, 3),
                "ego_speed_kmh": round(ego_speed_kmh, 3),
                "speed_limit_kmh": round(self._speed_threshold_kmh, 3),
            })
            event.set_message(
                "Overspeed on speed bump near ({:.2f}, {:.2f}): {:.2f} km/h > {:.2f} km/h".format(
                    bump_x, bump_y, ego_speed_kmh, self._speed_threshold_kmh
                )
            )
            self.events.append(event)
