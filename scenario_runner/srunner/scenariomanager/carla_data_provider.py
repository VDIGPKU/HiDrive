...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def create_blueprint(model, rolename='scenario', color=None, actor_category="car", attribute_filter=None):
        ...
        try:
            ...
            if len(blueprints) == 0:
                raise ValueError(f"No blueprints found for {model}")
            ...
            bp_filter = "vehicle.*"
            new_model = _actor_blueprint_categories.get(actor_category, '')
            if new_model != '':
                bp_filter = new_model
            print("WARNING: Actor model {} not available. Using filter {}".format(model, bp_filter))
            fallback_blueprints = list(CarlaDataProvider._blueprint_library.filter(bp_filter))
            if len(fallback_blueprints) == 0:
                fallback_blueprints = list(CarlaDataProvider._blueprint_library.filter("vehicle.*"))
            if len(fallback_blueprints) == 0:
                print("ERROR: No vehicle blueprints available!")
                return None
            blueprint = CarlaDataProvider._rng.choice(fallback_blueprints)
            print(f"Using fallback vehicle: {blueprint.id}")
